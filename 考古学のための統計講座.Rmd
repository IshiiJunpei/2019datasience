...
```{r fig.width=8, fig.height=7,warning=FALSE,error=FALSE}
p<-pot%>%
...
```{r fig.width=8, fig.height=4,warning=FALSE,error=FALSE}
p<-pot%>%
...
```{r fig.width=8, fig.height=5,warning=FALSE,error=FALSE,message=FALSE}
p<-iron%>%
	ggplot(aes(x=刀身長))+
	geom_histogram()+
	theme_minimal()
...
```{r fig.width=8, fig.height=5,warning=FALSE,error=FALSE}
# 正規曲線作成
x<-seq(0, 60, 0.1)
...
なお、Rで円グラフ（Pie charts）のヘルプを見ると次のような記述があり、使用を推奨されていません。
...
「円グラフで表示できるデータは全てドットチャートで表現できます。このことは角度による不正確な判定のかわりに共通のスケールによる判断ができることを意味しています」（Cleveland 1985,p264）

## 構成比棒グラフ
構成比を比較するために使われるのが構成比棒グラフです。長さや位置によって視覚化されるため、正確な読み取りが可能です。
構成比棒グラフは比率を比較するには非常に優れたグラフ表現です。

```{r  fig.width=8, fig.height=3,warning=FALSE,error=FALSE}
p<-toj%>%
...
plot(p)
```
```{r fig.width=8, fig.height=2.5,warning=FALSE,error=FALSE}
# 分類群ごとにファセットして描画
p<-pot%>%
	ggplot(aes(x=口径,y=器高,colour=分類))+
		geom_point()+
