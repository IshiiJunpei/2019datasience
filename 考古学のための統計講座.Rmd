...
```{r fig.width=6, fig.height=5,warning=FALSE,error=FALSE}
p<-pot%>%
...
```{r fig.width=6, fig.height=4,warning=FALSE,error=FALSE}
p<-pot%>%
...
```{r fig.width=4, fig.height=5,warning=FALSE,error=FALSE}
p<-pot%>%
...
```{r fig.width=6, fig.height=4,warning=FALSE,error=FALSE,message=FALSE}
p<-iron%>%
	ggplot(aes(x=刀身長))+
	geom_histogram()+
	theme_minimal()
...
これは、散布図では絶対に表現できません。
上記のヒストグラムから、私たちは確信をもって、古代の刀剣に複数のサイズ規範があることだけは確信することができそうです。
...
遺跡ごとあるいは住居跡ごとに出土遺物の構成比を調べる際に使用する可視化手法を紹介します。もっとも大切なことは、**円グラフを使わない**ということです。

人間の目は線の長さや点の位置を把握することには長けていますが、面積の大小を認識するのは苦手です。
円グラフは面積で比率を表すので比率の比較には向いていないのです。

```{r  fig.width=4, fig.height=4,warning=FALSE,error=FALSE}
toj_pie<-toj%>%group_by(器種)%>%summarise(点数=sum(点数))
pie(toj_pie$点数,labels=toj_pie$器種)
```

特に３D円グラフは目の錯覚を利用して、特定の値を大きく（小さく）見せるための手法です。公文書や学術的な報告で使うべきものではありません。
![](fig/threed_pie.png)

ちなみにRでパイチャートのヘルプを見ると次のような記述があります。
...
円グラフは情報の可視化としては悪い手法です。人間の目は直線的な形状の判断には優れていますが、面の比較は苦手です。円グラフで表現できるデータは棒グラフやドットチャートで表現するのが適切です。

「円グラフで表示できるデータは全てドットチャートで表現できます」（Cleveland 1985,p264）

## 構成比棒グラフ
構成比を比較するために使われるのが構成比棒グラフです。長さや位置によって視覚化されるため、正確な読み取りが可能です。
構成比棒グラフは比率を比較するには非常に優れたグラフ表現です。

```{r  fig.width=6, fig.height=3,warning=FALSE,error=FALSE}
p<-toj%>%
...
```{r fig.width=10, fig.height=6,warning=FALSE,error=FALSE}
coe<-lm(口径 ~ 器高,data=subset(pot,分類=="A型"))%>%summary()
...
単に当てはめるだけなら、下のような多項式の曲線でも良いわけです。
コンピュータに任せればこのような当てはめは自動的に行ってくれます。しかし、ラインを当てはめることが目的ではありませんから、どのような数式でモデル化するべきかということは人間が判断します。
...
A型土器の口径と器高の関係は1次式で近似できそうだと判断しました。
ここからは、相関係数の算出、当てはめた一次式の統計的な妥当性を判断していくことになりますが、こうした統計手法を使う前提として、散布図によって二変量の関係を視覚化する必要があるのです。


## 散布図が表現するのは分布や偏りではない

土器の「口径」と「器高」を分類ごとに示して、回帰直線をあてはめました。
下のグラフを見ると、分類によって回帰直線の傾きが異なることや「口径」、「器高」の分布が異なることがわかります。

このようなグラフを描いて、分類群によって「口径」や「器高」が異なることを示そうと考えるかもしれません。
しかし、群ごとの分布の違いを可視化するには、箱ひげ図や密度図が適切です。
この目的に、あえて散布図を用いる必然性はありません。
あくまでも***二変量の関係*の可視化が散布図を描く目的です。
...
## 散布図行列
古代の刀剣を対象としたデータで散布図を作成します。
追求すべきテーマは「刀身長と他の属性との因果関係」とします。
...
刀身長との相関の強弱を判断することで、古代剣製作にかかる範意識を読み取ることが可能かもしれません。
...
render("考古学のための統計講座.Rmd", encoding = "utf8")
```

![この 記事 は クリエイティブ・コモンズ 表示 4.0下に提供されています](fig/ccby.png)
